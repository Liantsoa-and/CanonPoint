...
{
    [Fact]
    public void GetValidatedLines_WhenAdverseInvulnerableWallOnLine_RejectsLineAndScoreStaysZero()
    {
        ...
        // Build a candidate line manually to validate the wall filter.
        gameState.GetCell(3, 1).SetOwner(PlayerSide.Player1);
        gameState.GetCell(3, 2).SetOwner(PlayerSide.Player1);
        gameState.GetCell(3, 3).SetOwner(PlayerSide.Player2);
        gameState.GetCell(3, 3).SetInvulnerable(true);
        gameState.GetCell(3, 4).SetOwner(PlayerSide.Player1);
        gameState.GetCell(3, 5).SetOwner(PlayerSide.Player1);
        ...
        var validated = validation.GetValidatedLines(gameState, [line], PlayerSide.Player1);

        Assert.Empty(validated);
        Assert.Equal(0, gameState.ScorePlayer1);
        Assert.Equal(0, gameState.ScorePlayer2);
        Assert.DoesNotContain("3:1|3:2|3:3|3:4|3:5", gameState.CountedLineHashes);
    }

    [Fact]
    public void GetValidatedLines_WhenNoAdverseWall_ValidatesLineAndIncrementsScore()
    {
        var factory = new GameStateFactory();
